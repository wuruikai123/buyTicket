...
ALTER TABLE `order_item`
  ...
  ADD COLUMN `buyer_phone` varchar(20) NULL COMMENT '购票人手机号' AFTER `buyer_id_card`,
  ADD COLUMN `ticket_status` int NULL DEFAULT 1 COMMENT '子票状态(1待使用,2已使用,5退款中,6已退款)' AFTER `buyer_phone`,
...
SET oi.buyer_name = COALESCE(oi.buyer_name, o.contact_name),
    ...
    oi.buyer_phone = COALESCE(oi.buyer_phone, o.contact_phone)
WHERE oi.buyer_name IS NULL OR oi.buyer_id_card IS NULL OR oi.buyer_phone IS NULL;
